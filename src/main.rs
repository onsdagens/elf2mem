...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut text_path: Vec<PathBuf> = vec![];
    let mut text_f = vec![];
    for i in 0..4 {
        let mut path = if let Some(out_dir) = cli.out_dir.clone() {
            out_dir
        } else {
            PathBuf::new()
        };
        path.push(path.with_file_name(format!("data_{}.mem", i)));
        ro_path.push(path.clone());
        ro_f.push(fs::File::create(path)?);
        ...
        path.push(path.with_file_name(format!("text_{}.mem", i)));
        text_path.push(path.clone());
        text_f.push(fs::File::create(path)?);
    }
    ...
    dump_text(&elf, text_section, data, &mut text_f)?;
...
fn dump_text(
    ...
    for (i, mut f) in text_f.iter().enumerate() {
        writeln!(f, "// section {:?}[{}]", sh.get_name(elf)?, i)?;
        writeln!(f, "@{:x?}", sh.address())?;
    }

    let d = &data[sh.offset() as usize..(sh.offset() + sh.size()) as usize];

    for (i, byte) in d.iter().enumerate() {
        write!(text_f[i % 4], "{:x} ", byte)?;
    }
